...
/// Id used for the plist entry
const DMD_IDENTIFIER: &str = "me.niclasve.dark-mode-daemon";
...
fn get_autostart_file_path(mkdirs: bool) -> Result<PathBuf, Error> {
    let Some(home_dir) = std::env::home_dir() else {
        return Err(Error::InvalidConfigDir(
            "Home dir could not be found".into(),
        ));
    };

    let launch_agents_dir = home_dir.join("Library").join("LaunchAgents");
    if mkdirs && !launch_agents_dir.exists() {
        fs::create_dir(&launch_agents_dir).map_err(Error::CouldNotCreateAutostartDir)?;
    }

    if !launch_agents_dir.is_dir() {
        let message = format!("'{}' is not a directory!", launch_agents_dir.display());
        return Err(Error::InvalidAutostartDir(message));
    }

    return Ok(launch_agents_dir.join(format!("{DMD_IDENTIFIER}.plist")));
}
